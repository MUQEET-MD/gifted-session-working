...
function giftedId(num = 22) {
  ...
  return `Muqeet-md~${result}`;
}

async function downloadCreds(sessionId) {  
  try {
    if (!sessionId.startsWith('Muqeet-md~')) {
      throw new Error('Invalid SESSION_ID: It must start with "Muqeet-md~"');
    }

    const response = await axios.get(
      `${SESSIONS_API_URL}/api/downloadCreds.php/${sessionId}`,
      {
        headers: { '6z6pNyW5ti81MkaZj361FsOvk': SESSIONS_API_KEY },
        timeout: 10000
      }
    );
